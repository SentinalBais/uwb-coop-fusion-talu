...
'''
ElevationMap Class for UWBThresholdUpdater Package  
'''

class ElevationMap(object):
    def __init__(self, delta_x=0, delta_y=0, cell_size=0.2, z_min=1, z_max=-2, bin_min_cap=2, emap=[], o_row_i=0, o_col_i=0, ec=-1):

        # instantiation if given a map of pre-allocated elevations
        if len(emap) > 0:
            self.z_min = z_min
            self.z_max = z_max
            self.g_z_max = -100
            ...
            # CONSTANTS
            EDGE_PADDING = 20
            # intilise the elevation 
            # coordinate of origin
            self.num_rows = emap.shape[0] 
            self.num_cols = emap.shape[1] 
            self.origin_x = o_row_i
            self.origin_y = o_col_i
            ...
            # load the elevation map with the provided data 
            for row_i, row in enumerate(emap):
                for col_i, elv in enumerate(row):
                    # convert elevation values into meters 
                    elv_m = elv/100.0
                    # check if current value known
                    if elv_m != ec:
                        self.map[row_i, col_i].set_z(elv_m)
                        self.map[row_i, col_i].known = True
                    else:
                        self.map[row_i, col_i].known = False
        else:
            # normal instanitation of elevation map with no intital data
            self.z_min = z_min
            self.z_max = z_max
            self.g_z_max = -100
            self.g_z_min = 100
